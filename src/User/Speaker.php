...
class Speaker
{
    const COLOR_ERROR = '#FF8F80';
    const COLOR_SUCCESS = '#90F47F';
    const COLOR_TEMPORARY = '#FFB680';
    const COLOR_INFO = '#d3a8ff';
    ...
    public function __construct()
    {
        if (!empty($_GET['speaker'])) {
            $content = Sanitize::sanitize($_GET['speaker']);
            $this->messages[] = [
                'content' => $content,
                'background' => null,
                'special' => null
    ...
    public function add($content, $background = null, $special = null)
    {
        $this->messages[] = [
            'content' => $content,
